...
const OCEANBASE_SLOW_SQL_PLAYBOOK = `
目标:
按照下面的步骤查找OceanBase数据库中的慢查询,并分析原因
步骤一:
使用工具getSlowQuery查找执行超过特定时间且排名前十的sql_id和query_sql
步骤二:
使用工具getExplain采集下sql语句的逻辑执行计划
步骤三:
根据第一步得到的结果,使用工具getPlan搜索计划信息
步骤四:
根据拿到的plan_id,使用工具getPhysicalPlan查询物理执行计划
最后:
在你完成之后,对你的发现做一个总结:慢查询的SQL语句,慢的原因,如果需要加索引的话把DDL语句写出来,
还要说一下用户会从该索引获得什么样的改进
`;
const OCEANBASE_SLOW_QUERIES_PLAYBOOK_NEW = `
...
export function getPlaybook(name: string): string {
  switch (name) {
    ...
    case 'oceanbaseSlowSql':
      return OCEANBASE_SLOW_SQL_PLAYBOOK;
    case 'oceanbaseSlowQuery':
      return OCEANBASE_SLOW_QUERIES_PLAYBOOK_NEW;
...
export function getBuiltInPlaybooks(): Playbook[] {
  return [
    ...
    // {
    //   name: 'oceanbase慢SQL',
    //   description: '查询OceanBase的慢SQL,并分析原因',
    //   content: OCEANBASE_SLOW_SQL_PLAYBOOK,
    //   isBuiltIn: true
    // },
    {
      name: 'oceanbaseSlowQuery',
      description: '查询OceanBase的慢查询，并分析原因',
      content: OCEANBASE_SLOW_QUERIES_PLAYBOOK_NEW,
      isBuiltIn: true
    },
    {
...
export function getPlaybookDetails(name: string): Playbook | undefined {
  console.log('getPlaybookDetails');
  console.log(name);
